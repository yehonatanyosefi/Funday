<template>
  <section class="task-preview" v-if="cmpOrder?.length">
    <div
      class="task-sticky task task-title-container"
      :class="{ 'first-task-sticky': idx === 0 }"
    >
      <div
        class="task-preview-color"
      ...
      <RouterLink
        :to="'/board/' + boardId + '/main-table/task/' + task.id"
        class="task-details-icon"
      >
