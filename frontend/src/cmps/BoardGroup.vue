...
export default {
  ...
  methods: {
    ...
    onTaskDrop(dropPayload) {
      ...
      const removedId = this.group.tasks.find(
        (task, idx) => idx === removedIndex
      ).id;
      const addedId = this.group.tasks.find(
        (task, idx) => idx === addedIndex
      ).id;
    ...
    startDate() {
      const startDate = Math.min(
        ...this.group.tasks
          .filter(
            (task) =>
              typeof task.timeline?.startDate === "number" &&
              task.timeline.startDate > 0
          )
    ...
    dueDate() {
      const dueDate = Math.max(
        ...this.group.tasks
          .filter(
            (task) =>
              typeof task.timeline?.dueDate === "number" &&
              task.timeline.dueDate > 0
          )
