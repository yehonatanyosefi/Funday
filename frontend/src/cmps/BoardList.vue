        ...
        <Draggable v-for="board in boardList" :key="board._id">
          <li
            ...
            :class="{
              ...
              rename: currBoardId === board._id && isRename,
            }"
            @click="setBoard(board._id)"
          >
            <Board class="svg-icon board-svg" height="19px" width="19px" />
            <span v-if="(isRename && currBoardId !== board._id) || !isRename">{{
              board.title
            }}</span>
            ...
            <div
              v-if="isModalOpen && currBoardId === board._id"
              class="modal"
              v-click-outside.stop="closeModal"
            >
              <div
                @click.stop="toggleRename(board._id)"
...
export default {
  ...
  methods: {
    ...
    toggleRename(boardId) {
      this.title = this.boardList.find(
        (listItem) => listItem._id === boardId
      ).title;
      this.isRename = true;
      this.closeModal();
    },
    ...
    async duplicateBoard() {
      try {
        let duplicatedBoard = JSON.parse(
    ...
    onBoardDrop(dropPayload) {
      ...
      const removedId = this.boardList.find(
        (board, idx) => idx === removedIndex
      )._id;
      const addedId = this.boardList.find(
        (board, idx) => idx === addedIndex
      )._id;
