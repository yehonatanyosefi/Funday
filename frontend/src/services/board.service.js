...
async function save(boardId = null, type = 'task', payload, groupId = null) {
  ...
  switch (type) {
    ...
    case 'group':
      ...
      break
    case 'member':
      const memberIdx = type !== 'board' ? board?.members.findIndex((member) => member._id === payload._id) : -1
      if (memberIdx > -1) {
...
function setAdvanceFilter(board, advanceFilter) {
  board.groups = board.groups.reduce((groupArr, group) => {
    const groupInclude = advanceFilter.group?.length
      ? advanceFilter.group.includes(group.title)
      : true
    if (groupInclude) {
      group.tasks = group.tasks.reduce((taskArr, task) => {
        ...
        const personInclude = advanceFilter.person.length
          ? advanceFilter.person.some((item) =>
            task.person.some((person) => person === item._id)
          )
          : true
