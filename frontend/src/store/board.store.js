...
export const boardStore = {
  ...
  mutations: {
    ...
    removeTask(state, { ids }) {
      ...
      const taskIdx = state.board.groups[groupIdx].tasks.findIndex(
        (task) => task.id === taskId
      )
  ...
  actions: {
    ...
    async removeGroup({ commit, dispatch }, { payload }) {
      try {
        ...
        const updatedBoard = await boardService.remove(
          { boardId, groupId },
          'group'
        )
        dispatch({ type: 'setAndFilterBoard', board: updatedBoard })
    ...
    async saveGroupAtt({ state, dispatch }, { payload }) {
      try {
        ...
        const group = JSON.parse(
          JSON.stringify(
            state.board.groups.find((group) => group.id === groupId)
          )
        )
    ...
    async setBoardById({ dispatch }, { boardId }) {
      try {
        const board = await boardService.getById(boardId)
        dispatch({ type: 'setAndFilterBoard', board })
        return board
      } catch (err) {
        console.log('Store: Error in setBoardById', err)
      }
    },
    ...
    async getFirstBoard({ dispatch, state }, { params }) {
      ...
      dispatch({ type: 'setBoardById', boardId })
      return boardId
    },
